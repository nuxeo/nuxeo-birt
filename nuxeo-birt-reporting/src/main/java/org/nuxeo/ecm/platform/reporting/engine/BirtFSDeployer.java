...
/**
 * Because BIRT relies on Equinox deployment, this class is used to create an OSGi/Equinox sandbox on the filesystem and start Birt deployment
 *
...
public class BirtFSDeployer implements IPlatformContext {
    ...
    protected void deployBirtPlatform()
    {
        if ( platformPath == null ){
            ...
            if (Framework.isTestModeSet()) {
                ...
                String dirPath = new Path(System.getProperty("java.io.tmpdir")).append("birt-fs" + System.currentTimeMillis()).toString();
            ...
            try {
                ...
                if (! Framework.isTestModeSet()) {
                    ...
                    String targetPath = birtPlatformFolder.getAbsolutePath() + JDBC_JAR_DIR;
                    for (String driverJar : driverJars) {
                        FileUtils.copyFile(new File(driverJar), new File(targetPath));
                    }
                }
            } catch (Exception e) {
    ...
    protected void copyResources( String resourcePath, File dir ) throws Exception
    {
        URL url = Thread.currentThread().getContextClassLoader().getResource(resourcePath);
        if ("jar".equals(url.getProtocol())) {
            String jarPath = url.getFile().split("!")[0].replace("file:", "");
            InputStream zipStream = ZipUtils.getEntryContentAsStream(new File(jarPath), BIRT_ZIP_NAME);
